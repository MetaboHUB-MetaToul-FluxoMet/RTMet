...
# def store_metabolites(met_dict, dataframe_path):
#     """
#     """

#     dataframe = pd.read_csv(dataframe_path, sep="\t")
#     for metabolite in dataframe["Precursor Name"].unique():
#         met_dict[metabolite] = {"datetime": dataframe["Datetime"].loc[dataframe["Precursor Name"] == metabolite].tolist(),
...
if "met_dict" not in st.session_state:
    st.session_state.met_dict = met_dict

dataframe = pd.read_csv("/home/kouakou/Documents/all_features.tsv", sep="\t")
for metabolite in dataframe["Precursor Name"].unique():
    met_dict[metabolite] = {"datetime": dataframe["Datetime"].loc[dataframe["Precursor Name"] == metabolite].tolist(),
                            "intensity": dataframe["Intensity"].loc[dataframe["Precursor Name"] == metabolite].tolist(),
                            "mz": dataframe["MzQuery"].loc[dataframe["Precursor Name"] == metabolite].tolist()}
...
if st.session_state.metabolite_checkbox:
    ...
    for i in st.session_state.metabolite_checkbox:
        if st.session_state.metabolite_checkbox[i]:
            ...
            df = pd.DataFrame({
                    "Datetime": met_dict[i]["datetime"],
                    "Intensity": met_dict[i]["intensity"]
                })
            with graph:
