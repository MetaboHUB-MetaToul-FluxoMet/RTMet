...
class RoseDirective(ObjectDescription):
    ...
    def add_ref_context(self, key):
        ...
        * The variable will be automatically removed in ``after_content()``
        ...
        ref_context[context_var] = self.process_name(self.arguments[0].strip())[0]
...
class RoseDomain(Domain):
    ...
    def get_objects(self):
        ...
        for obj_name, (doc_name, _) in list(self.data["objects"].items()):
            ...
            try:
                ...
                LOGGER.warning('Could not parse object reference for "%s"' % ref_name)
                continue
    ...
    def validate_external_xref(env, typ, target, node, intersphinx_mapping):
        ...
        try:
            ...
            cross_target = env.config.intersphinx_mapping[intersphinx_mapping][0]
        except KeyError:
            LOGGER.warning(
                'Could not find inter-sphinx mapping for "%s"' % intersphinx_mapping
            )
            return False
        except AttributeError:
            LOGGER.warning("inter-sphinx required for cross-project " "references.")
            return False
        try:
            ...
            LOGGER.warning(
                'No Ref for "rose:%s:`%s:%s`"' % (typ, intersphinx_mapping, target),
                location=node,
            )
            return False
        return True
    ...
    def resolve_xref(self, env, fromdocname, builder, typ, target, node, contnode):
        ...
        if match:
            ...
            self.validate_external_xref(env, typ, target, node, intersphinx_mapping)
        ...
        if typ in ["app", "file"]:
            ...
            reference_namespace = tokenise_namespace("rose:%s:%s" % (typ, target))
            if reference_namespace[0][0] in ["rose:app", "rose:file"]:
                ...
                if relative_to_conf:
                    # Target is relative to the context conf_file.
                    namespace_tokens = context_namespace[:1] + reference_namespace
                else:
...
class RoseAutoDirective(Directive):
    ...
    def run(self):
        ...
        try:
            ...
            LOGGER.error('Syntax error in Rose configuration file "%s".' % filename)
            raise

        nodes = []
        nodes.append(
            addnodes.highlightlang(lang="rose", force=False, linenothreshold=20)
        )
        ...
        nodes.append(
            addnodes.highlightlang(lang="bash", force=False, linenothreshold=20)
        )
